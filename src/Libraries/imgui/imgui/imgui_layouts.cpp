...
{
    ImGuiLayoutType         LayoutType{0};
    ImGuiLayoutType         ParentLayoutType{0};       // Layout type of parent window at the time of Begin()
    ImGuiLayout*            CurrentLayout{nullptr};
    ImGuiLayoutItem*        CurrentLayoutItem{nullptr};
    ImVector<ImGuiLayout*>  LayoutStack{};
    ImGuiStorage            Layouts{};
};

static LayoutStorage& GetLayoutStorage(ImGuiWindow* window = NULL)
{
    static const ImGuiID LayoutStorageId = 4567880954;
    ...
    LayoutStorage* storage = (LayoutStorage*)window->StateStorage.GetVoidPtr(LayoutStorageId);

    if (!storage)
    {
        storage = IM_NEW(LayoutStorage)();
        window->StateStorage.SetVoidPtr(LayoutStorageId, storage);
    }
    return *storage;
}
...
static ImGuiLayout* ImGui::FindLayout(ImGuiID id, ImGuiLayoutType type)
{
    ...
    auto& DC = GetLayoutStorage();
    ImGuiLayout* layout = (ImGuiLayout*)DC.Layouts.GetVoidPtr(id);
    if (!layout)
        return NULL;
...
static ImGuiLayout* ImGui::CreateNewLayout(ImGuiID id, ImGuiLayoutType type, ImVec2 size)
{
    ...
    DC.Layouts.SetVoidPtr(id, layout);
...
static void ImGui::EndLayout(ImGuiLayoutType type)
{
   ImGuiWindow* window = GetCurrentWindow();
    auto& DC = GetLayoutStorage();
    IM_ASSERT(DC.CurrentLayout);
    IM_ASSERT(DC.CurrentLayout->Type == type);

    ImGuiLayout* layout = DC.CurrentLayout;
    ...
    if (DC.CurrentLayoutItem)
        current_layout_item_max = ImMax(DC.CurrentLayoutItem->MeasuredBounds.Max, layout->StartPos + new_size);
    ...
    if (DC.CurrentLayoutItem)
        DC.CurrentLayoutItem->MeasuredBounds.Max = current_layout_item_max;
...
static void ImGui::PushLayout(ImGuiLayout* layout)
{
    ...
    if (layout)
    {
        layout->Parent = DC.CurrentLayout;
        if (layout->Parent != NULL)
            layout->ParentItemIndex = layout->Parent->CurrentItemIndex;
        if (DC.CurrentLayout)
        {
            layout->NextSibling = DC.CurrentLayout->FirstChild;
            layout->FirstChild = NULL;
            DC.CurrentLayout->FirstChild = layout;
        }
        else
        {
    ...
    DC.LayoutStack.push_back(layout);
    DC.CurrentLayout = layout;
    DC.CurrentLayoutItem = NULL;
}
...
static void ImGui::PopLayout(ImGuiLayout* layout)
{
    ...
    IM_ASSERT(!DC.LayoutStack.empty());
    IM_ASSERT(DC.LayoutStack.back() == layout);

    DC.LayoutStack.pop_back();

    if (!DC.LayoutStack.empty())
    {
        DC.CurrentLayout = DC.LayoutStack.back();
        DC.CurrentLayoutItem = &DC.CurrentLayout->Items[DC.CurrentLayout->CurrentItemIndex];
    }
    else
    {
        DC.CurrentLayout = NULL;
        DC.CurrentLayoutItem = NULL;
    }
}
...
static ImGuiLayoutItem* ImGui::GenerateLayoutItem(ImGuiLayout& layout, ImGuiLayoutItemType type)
{
    ...
    auto& DC = GetLayoutStorage(g.CurrentWindow);
    DC.CurrentLayoutItem = &layout.Items[layout.CurrentItemIndex];
...
void ImGui::Spring(float weight/* = 1.0f*/, float spacing/* = -1.0f*/)
{
    ...
    AddLayoutSpring(*DC.CurrentLayout, weight, spacing);
}
